	...
	protected virtual void SetupScene(bool a_bIsMainSetup) {
		...
		if(a_bIsMainSetup) {
			...
			m_oCanvasDict.GetValueOrDefault(EKey.POPUP_UIS_CANVAS).ExSetSortingOrder(new STSortingOrderInfo() {
				m_nOrder = this.UIsCanvasSortingOrderInfo.m_nOrder + (sbyte.MaxValue * KCDefine.B_VAL_1_INT),
			...
			m_oCanvasDict.GetValueOrDefault(EKey.TOPMOST_UIS_CANVAS).ExSetSortingOrder(new STSortingOrderInfo() {
				m_nOrder = this.UIsCanvasSortingOrderInfo.m_nOrder + (sbyte.MaxValue * KCDefine.B_VAL_2_INT),
				m_oLayer = this.UIsCanvasSortingOrderInfo.m_oLayer
			});
			...
			if(m_oCameraDict.GetValueOrDefault(EKey.MAIN_CAMERA) != null) {
				...
				if(oPostProcessingVolume != null && CSceneManager.QualityLevel != EQualityLevel.NONE) {
					...
					var oResult = oPostProcessingSettingsPathDict.ExFindVal((a_oPostProcessingSettingsPath) => oPostProcessingVolume.sharedProfile != null && a_oPostProcessingSettingsPath.ExGetFileName(false).Equals(oPostProcessingVolume.sharedProfile.name));
					...
					// 포스트 프로세싱 설정이 없을 경우
					if((!bIsValid01 || (bIsValid02 && oResult.Item1)) && CAccess.IsExistsRes<PostProcessProfile>(oPostProcessingSettingsPathDict.GetValueOrDefault(CSceneManager.QualityLevel, string.Empty), true)) {
...
#if POST_PROCESSING_MODULE_ENABLE
						CFactory.RemoveObj(this.GetAdditionalCameras()[i].GetComponentInChildren<PostProcessVolume>(), false, false);
#endif // #if POST_PROCESSING_MODULE_ENABLE
					}
				} catch(System.Exception oException) {
	...
	private void DoSetupCanvas(Canvas a_oCanvas, bool a_bIsEnableAssert = true) {
		...
		if(a_oCanvas != null) {
			...
			a_oCanvas.ExSetSortingOrder(new STSortingOrderInfo() {
				m_nOrder = a_oCanvas.sortingOrder,
				m_oLayer = (a_oCanvas.renderMode == RenderMode.ScreenSpaceOverlay) ? KCDefine.U_SORTING_L_DEF : a_oCanvas.sortingLayerName
			});
