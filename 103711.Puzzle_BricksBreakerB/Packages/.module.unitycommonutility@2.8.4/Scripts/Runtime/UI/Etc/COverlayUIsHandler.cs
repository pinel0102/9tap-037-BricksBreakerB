	...
	public partial class COverlayUIsHandler : CComponent {
		...
		public virtual void Show(bool a_bIsAni = true) {
			...
			if(a_bIsAni && !this.IsIgnoreAni) {
				m_oAniDict.ExReplaceVal(EKey.SHOW_ANI, CFactory.MakeSequence(this.Params.m_oFuncCallbackDict.GetValueOrDefault(ECallback.MAKE_SHOW_ANI)?.Invoke(this), (a_oSender) => this.OnCompleteShowAni()));
			} else {
		...
		public virtual void Close(bool a_bIsAni = true) {
			// 애니메이션 모드 일 경우
			if(a_bIsAni && !this.IsIgnoreAni) {
				m_oAniDict.ExReplaceVal(EKey.CLOSE_ANI, CFactory.MakeSequence(this.Params.m_oFuncCallbackDict.GetValueOrDefault(ECallback.MAKE_CLOSE_ANI)?.Invoke(this), (a_oSender) => this.OnCompleteCloseAni()));
			} else {
