	...
	public abstract partial class CStartSceneManager : CSceneManager {
		...
		public override void Awake() {
			...
			if(CSceneManager.IsInit) {
				...
				m_oUIsDict.GetValueOrDefault(EKey.LOADING_GAUGE).SetActive(!this.IsIgnoreLoadingGauge);
				m_oUIsDict.GetValueOrDefault(EKey.LOADING_GAUGE).transform.localPosition = this.LoadingGaugePos;
				// 객체를 설정한다 }

				// 텍스트를 설정한다 {
				CFunc.SetupComponents(new List<(EKey, string, GameObject, GameObject)>() {
					(EKey.LOADING_TEXT, $"{EKey.LOADING_TEXT}", m_oUIsDict.GetValueOrDefault(EKey.LOADING_GAUGE), CResManager.Inst.GetRes<GameObject>(KCDefine.SS_OBJ_P_LOADING_TEXT))
				}, m_oTextDict);

				m_oTextDict.GetValueOrDefault(EKey.LOADING_TEXT).transform.localPosition = this.LoadingTextPos;
				// 텍스트를 설정한다 }
		...
		protected virtual void OnReceiveStartSceneEvent(EStartSceneEvent a_eEvent) {
			...
			m_oAniDict.ExAssignVal(EKey.LOADING_GAUGE_ANI, this.StartLoadingGaugeAni(m_oGaugeHandlerDict.GetValueOrDefault(EKey.LOADING_GAUGE_HANDLER), (a_fVal) => this.UpdateUIsState(), null, m_oGaugeHandlerDict.GetValueOrDefault(EKey.LOADING_GAUGE_HANDLER).Percent, fPercent, KCDefine.U_DURATION_ANI * KCDefine.B_VAL_2_REAL));
