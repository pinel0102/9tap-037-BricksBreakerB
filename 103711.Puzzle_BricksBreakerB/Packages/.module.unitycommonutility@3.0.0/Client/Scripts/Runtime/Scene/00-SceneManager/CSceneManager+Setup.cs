	...
	protected virtual void SetupScene(bool a_bIsMainSetup) {
		...
		if(a_bIsMainSetup) {
			...
			m_oCanvasDict.GetValueOrDefault(EKey.POPUP_UIS_CANVAS).ExSetSortingOrder(new STSortingOrderInfo() {
				m_nOrder = this.UIsCanvasSortingOrderInfo.m_nOrder + KCDefine.U_SORTING_O_SCREEN_POPUP_UIS, m_oLayer = this.UIsCanvasSortingOrderInfo.m_oLayer
			});

			m_oCanvasDict.GetValueOrDefault(EKey.TOPMOST_UIS_CANVAS).ExSetSortingOrder(new STSortingOrderInfo() {
				m_nOrder = this.UIsCanvasSortingOrderInfo.m_nOrder + KCDefine.U_SORTING_O_SCREEN_TOPMOST_UIS, m_oLayer = this.UIsCanvasSortingOrderInfo.m_oLayer
			});
			// 캔버스 순서를 설정한다 }
