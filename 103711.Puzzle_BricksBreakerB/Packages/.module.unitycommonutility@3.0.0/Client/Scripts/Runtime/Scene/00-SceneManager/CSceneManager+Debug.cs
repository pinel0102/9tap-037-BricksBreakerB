...
public abstract partial class CSceneManager : CComponent {
	...
	private void SetupDebugUIs(GameObject a_oUIsObjs, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null && CSceneManager.ScreenStaticDebugText != null) {
			...
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_01, CAccess.DeviceScreenSize.x, CAccess.DeviceScreenSize.y);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_02, CSceneManager.CanvasSize.x, CSceneManager.CanvasSize.y);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_03, CSceneManager.UpSafeAreaOffset, CSceneManager.DownSafeAreaOffset, CSceneManager.LeftSafeAreaOffset, CSceneManager.RightSafeAreaOffset);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_04, (EQualityLevel)QualitySettings.GetQualityLevel(), Application.targetFrameRate);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_05, KCDefine.B_DIR_P_WRITABLE);

#if ADS_MODULE_ENABLE
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_06, CAccess.ScreenDPI, CAccess.GetBannerAdsHeight(KCDefine.U_SIZE_BANNER_ADS.y));
