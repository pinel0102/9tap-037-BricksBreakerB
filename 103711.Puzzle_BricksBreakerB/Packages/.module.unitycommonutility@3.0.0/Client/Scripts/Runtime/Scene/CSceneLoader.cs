...
public partial class CSceneLoader : CSingleton<CSceneLoader> {
	...
	#region 프로퍼티
	public string PrevActiveSceneName => m_oStrDict.GetValueOrDefault(EKey.PREV_ACTIVE_SCENE_NAME, string.Empty);
	public string AwakeActiveSceneName => m_oStrDict.GetValueOrDefault(EKey.AWAKE_ACTIVE_SCENE_NANE, string.Empty);
	#endregion // 프로퍼티

	#region 함수
	/** 이전 액티브 씬 이름을 변경한다 */
	public void SetPrevActiveSceneName(string a_oSceneName) {
		m_oStrDict.ExReplaceVal(EKey.PREV_ACTIVE_SCENE_NAME, a_oSceneName);
	}

	/** 최초 액티브 씬 이름을 변경한다 */
	public void SetAwakeActiveSceneName(string a_oSceneName) {
		m_oStrDict.ExReplaceVal(EKey.AWAKE_ACTIVE_SCENE_NANE, a_oSceneName);
	}
