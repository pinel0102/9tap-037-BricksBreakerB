...
public static partial class CMenuHandler {
	...
	public static void ExportImgToPNG() {
	...
	public static void ExportDefTexToPNG() {
	...
	/** 텍스처를 저장한다 */
	private static void SaveTex(string a_oFilePath, Texture2D a_oTex2D) {
		var oTex = new Texture2D(a_oTex2D.width, a_oTex2D.height, a_oTex2D.format, KCDefine.B_VAL_1_INT, true);
		oTex.ExSetPixels(a_oTex2D.GetPixels(KCDefine.B_VAL_0_INT).ToList());
