...
public partial class CIndicatorManager : CSingleton<CIndicatorManager> {
	...
	public override void OnDestroy() {
		...
		try {
			...
			if(CSceneManager.IsAppRunning) {
				m_oAniDict[EKey.INDICATOR_ANI]?.Kill();
			}
	...
	public void SetInfoText(string a_oStr) {
		...
		m_oTextDict[EKey.INFO_TEXT]?.ExSetText(a_oStr, stFontSetInfo, false);
	}

	/** 인디케이터를 출력한다 */
	public void Show(bool a_bIsResetRefCount = false) {
		int nRefCount = m_oIntDict[EKey.REF_COUNT] + KCDefine.B_VAL_1_INT;
		m_oIntDict[EKey.REF_COUNT] = a_bIsResetRefCount ? KCDefine.B_VAL_1_INT : Mathf.Clamp(nRefCount, KCDefine.B_VAL_0_INT, int.MaxValue);

		// 인디케이터 시작이 가능 할 경우
		if(m_oIntDict[EKey.REF_COUNT] > KCDefine.B_VAL_0_INT) {
			...
			m_oTextDict[EKey.INFO_TEXT].rectTransform.pivot = KCDefine.B_ANCHOR_UP_CENTER;
			m_oTextDict[EKey.INFO_TEXT].rectTransform.anchorMin = KCDefine.B_ANCHOR_MID_CENTER;
			m_oTextDict[EKey.INFO_TEXT].rectTransform.anchorMax = KCDefine.B_ANCHOR_MID_CENTER;
			m_oTextDict[EKey.INFO_TEXT].rectTransform.anchoredPosition = new Vector3(KCDefine.B_VAL_0_REAL, CSceneManager.CanvasSize.y / -KCDefine.B_VAL_4_REAL, KCDefine.B_VAL_0_REAL);
			// 텍스트를 설정한다 }
	...
	public void Close(bool a_bIsResetRefCount = false) {
		int nRefCount = m_oIntDict[EKey.REF_COUNT] - KCDefine.B_VAL_1_INT;
		m_oIntDict[EKey.REF_COUNT] = a_bIsResetRefCount ? KCDefine.B_VAL_0_INT : Mathf.Clamp(nRefCount, KCDefine.B_VAL_0_INT, int.MaxValue);
