...
public abstract partial class CPopup : CComponent {
	...
	/** =====> UI <===== */
	protected virtual Image BGImg => m_oImgDict[EKey.BG_IMG];
	protected virtual Image BlindImg => this.BlindTouchResponder?.GetComponentInChildren<Image>();

	protected virtual Button CloseBtn => m_oBtnDict[EKey.CLOSE_BTN];

	/** =====> 객체 <===== */
	protected GameObject Contents => m_oUIsDict[EKey.CONTENTS];
	protected GameObject ContentsUIs => m_oUIsDict[EKey.CONTENTS_UIS];
	protected GameObject BlindTouchResponder => m_oUIsDict[EKey.BLIND_TOUCH_RESPONDER];
	#endregion // 프로퍼티
	...
	public override void Awake() {
		...
		CFunc.SetupButtons(new List<(EKey, string, GameObject, UnityAction)>() {
			(EKey.CLOSE_BTN, $"{EKey.CLOSE_BTN}", m_oUIsDict[EKey.CONTENTS], this.OnTouchCloseBtn)
		}, m_oBtnDict);
		...
		switch(this.AniType) {
			...
			case EAniType.SLIDE_RIGHT: {
				...
				if(this.AniType == EAniType.DROPDOWN) {
					m_oUIsDict[EKey.CONTENTS].transform.localPosition = new Vector3(KCDefine.B_VAL_0_REAL, CSceneManager.CanvasSize.y, KCDefine.B_VAL_0_REAL);
				} else {
					m_oUIsDict[EKey.CONTENTS].transform.localPosition = new Vector3((this.AniType == EAniType.SLIDE_LEFT) ? CSceneManager.CanvasSize.x : -CSceneManager.CanvasSize.x, KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL);
				}
	...
	public void SetBlindColor(Color a_stColor, bool a_bIsAni = true, float a_fDuration = KCDefine.U_DURATION_POPUP_ANI) {
		...
		if(a_bIsAni && !this.IsIgnoreAni) {
			...
			m_oAniDict.ExAssignVal(EKey.BLIND_ANI, this.BlindImg.DOColor(a_stColor, a_fDuration).SetUpdate(true));
	...
	public virtual void Show(System.Action<CPopup> a_oShowCallback, System.Action<CPopup> a_oCloseCallback) {
		// 객체가 존재 할 경우
		if(!this.IsDestroy && (!m_oBoolDict[EKey.IS_SHOW] && !m_oBoolDict[EKey.IS_CLOSE])) {
			m_oBoolDict[EKey.IS_SHOW] = true;
	...
	public virtual void Close() {
		// 출력 상태 일 경우
		if(!this.IsDestroy && (m_oBoolDict[EKey.IS_SHOW] && !m_oBoolDict[EKey.IS_CLOSE])) {
			m_oBoolDict[EKey.IS_CLOSE] = true;
	...
	protected virtual void SetupContents() {
		...
		if(!this.IsIgnoreRebuildLayout) {
			CSceneManager.ActiveSceneManager.RebuildLayouts(m_oUIsDict[EKey.CONTENTS]);
		}
	...
	protected virtual Tween MakeShowAni() {
		switch(this.AniType) {
			case EAniType.SCALE: return m_oUIsDict[EKey.CONTENTS].transform.DOScale(KCDefine.U_SCALE_POPUP, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.OutBack);
			case EAniType.DROPDOWN: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveY(KCDefine.B_VAL_0_REAL, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.OutBack);
			case EAniType.SLIDE_LEFT:
			case EAniType.SLIDE_RIGHT: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveX(KCDefine.B_VAL_0_REAL, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.OutBack);
		}
	...
	protected virtual Tween MakeCloseAni() {
		switch(this.AniType) {
			case EAniType.SCALE: return m_oUIsDict[EKey.CONTENTS].transform.DOScale(KCDefine.U_MIN_SCALE_POPUP, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.InBack);
			case EAniType.DROPDOWN: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveY(CSceneManager.CanvasSize.y, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.InBack);
			case EAniType.SLIDE_LEFT:
			case EAniType.SLIDE_RIGHT: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveX((this.AniType == EAniType.SLIDE_LEFT) ? -CSceneManager.CanvasSize.x : CSceneManager.CanvasSize.x, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.InBack);
		}
	...
	private void StartShowAni() {
		...
		if(!this.IsIgnoreAni && this.AniType != EAniType.NONE) {
			m_oAniDict.ExAssignVal(EKey.SHOW_ANI, CFactory.MakeSequence(this.MakeShowAni(), (a_oSender) => this.OnCompleteShowAni(), KCDefine.B_VAL_0_REAL, false, true));
		} else {
	...
	private void StartCloseAni() {
		...
		// 애니메이션 모드 일 경우
		if(!this.IsIgnoreAni && this.AniType != EAniType.NONE) {
			m_oAniDict.ExAssignVal(EKey.CLOSE_ANI, CFactory.MakeSequence(this.MakeCloseAni(), (a_oSender) => this.OnCompleteCloseAni(), KCDefine.B_VAL_0_REAL, false, true));
		} else {
	...
	private void DoShow(MonoBehaviour a_oSender) {
		// 출력 가능 할 경우
		if(!this.IsDestroy && !m_oBoolDict[EKey.IS_CLOSE]) {
			this.SetupContents();
			this.StartShowAni();
