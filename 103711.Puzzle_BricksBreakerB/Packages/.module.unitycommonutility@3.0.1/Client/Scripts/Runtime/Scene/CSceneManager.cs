...
public abstract partial class CSceneManager : CComponent {
	...
	public static Vector3 CanvasScale { get; private set; } = Vector3.zero;
	...
	public static Vector3 CanvasScaleSize => CSceneManager.CanvasSize.ExGetScaleVec(CSceneManager.CanvasScale);
				...
				if(this.IsActiveScene && CSceneManager.IsAppInit) {
					...
					if(!this.IsIgnoreBGTouchResponder) {
						...
						CFunc.SetupComponents(new List<(EKey, GameObject)>() {
							(EKey.BG_TOUCH_DISPATCHER, m_oUIsDict[EKey.BG_TOUCH_RESPONDER])
						}, m_oTouchDispatcherDict);

						m_oTouchDispatcherDict[EKey.BG_TOUCH_DISPATCHER]?.SetBeginCallback(this.OnTouchBegin);
						m_oTouchDispatcherDict[EKey.BG_TOUCH_DISPATCHER]?.SetMoveCallback(this.OnTouchMove);
						m_oTouchDispatcherDict[EKey.BG_TOUCH_DISPATCHER]?.SetEndCallback(this.OnTouchEnd);
						// 터치 전달자를 설정한다 }
					}
					...
					if(!this.IsIgnoreABTestUIs) {
						...
						try {
							...
							oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].spacing = KCDefine.B_VAL_4_REAL * KCDefine.B_VAL_5_REAL;

							(oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].transform as RectTransform).pivot = KCDefine.B_ANCHOR_UP_CENTER;
							(oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].transform as RectTransform).anchorMin = KCDefine.B_ANCHOR_UP_CENTER;
							(oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].transform as RectTransform).anchorMax = KCDefine.B_ANCHOR_UP_CENTER;

							var oContentsSizeFitter = oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].gameObject.ExAddComponent<ContentSizeFitter>();
							...
							CFunc.SetupComponents(new List<(string, string, GameObject, GameObject)>() {
								(KCDefine.U_OBJ_N_A_SET_BTN, KCDefine.U_OBJ_N_A_SET_BTN, oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].gameObject, CResManager.Inst.GetRes<GameObject>(KCDefine.U_OBJ_P_TMP_TEXT_BTN)),
								(KCDefine.U_OBJ_N_B_SET_BTN, KCDefine.U_OBJ_N_B_SET_BTN, oHLayoutGroupDict[KCDefine.U_OBJ_N_AB_T_UIS_SET_UIS].gameObject, CResManager.Inst.GetRes<GameObject>(KCDefine.U_OBJ_P_TMP_TEXT_BTN))
							}, oTextDict);

							oTextDict[KCDefine.U_OBJ_N_A_SET_BTN].fontSize = KCDefine.U_DEF_SIZE_FONT;
							oTextDict[KCDefine.U_OBJ_N_A_SET_BTN].ExSetText(CStrTable.Inst.GetStr(KCDefine.ST_KEY_C_A_SET_TEXT), stFontSetInfo);

							oTextDict[KCDefine.U_OBJ_N_B_SET_BTN].fontSize = KCDefine.U_DEF_SIZE_FONT;
							oTextDict[KCDefine.U_OBJ_N_B_SET_BTN].ExSetText(CStrTable.Inst.GetStr(KCDefine.ST_KEY_C_B_SET_TEXT), stFontSetInfo);
							// 텍스트를 설정한다 }
...
#if DEBUG || DEVELOPMENT_BUILD
			this.RebuildLayouts(CSceneManager.ScreenDebugUIs);
#endif // #if DEBUG || DEVELOPMENT_BUILD
	...
	public virtual void OnApplicationQuit() {
		this.DoQuitApp();
		CFunc.EnumerateComponents<CComponent>((a_oComponent) => { CFunc.RemoveObj(a_oComponent); return true; }, true);
	}
	...
	public override void OnUpdate(float a_fDeltaTime) {
		...
		if(CSceneManager.IsAppRunning) {
			// 레이아웃 보정이 가능 할 경우
			if(!this.IsIgnoreRebuildLayout && !m_oBoolDict[EKey.IS_REBUILD_LAYOUT]) {
				m_oBoolDict[EKey.IS_REBUILD_LAYOUT] = true;
				this.RebuildLayouts();
			}
	...
	public void RebuildLayouts() {
		this.gameObject.scene.ExEnumerateRootObjs((a_oObj) => {
			this.RebuildLayouts(a_oObj);
			return true;
		});
	}
	...
	public void RebuildLayouts(GameObject a_oObj) {
		...
		try {
			a_oObj?.GetComponentsInChildren<RectTransform>(true, oTransList);

			for(int i = oTransList.Count - KCDefine.B_VAL_1_INT; i >= KCDefine.B_VAL_0_INT; --i) {
	...
	protected virtual void OnUpdateScreenSize(Vector3 a_stScreenSize) {
		...
		this.ExLateCallFunc((a_oSender) => { this.SetupScene(); this.RebuildLayouts(); });
	}
	...
	private static void OnCloseTouchResponder(GameObject a_oTouchResponder, System.Action<GameObject> a_oCallback) {
		try {
			...
			CFunc.RemoveObj(a_oTouchResponder);
		}
	}
	#endregion // 클래스 함수
