...
public partial class CServicesManager : CSingleton<CServicesManager> {
...
#if UNITY_IOS && APPLE_LOGIN_ENABLE
		// 애플 로그인을 지원 할 경우
		if(m_oBoolDict[EKey.IS_INIT] && AppleAuthManager.IsCurrentPlatformSupported) {
...
#if UNITY_IOS && APPLE_LOGIN_ENABLE
		// 로그인 되었을 경우
		if(!m_oBoolDict[EKey.IS_INIT] || m_oBoolDict[EKey.IS_APPLE_LOGIN]) {
			CFunc.Invoke(ref a_oCallback, this, m_oBoolDict[EKey.IS_APPLE_LOGIN]);
		}
		// 애플 로그인을 지원 할 경우
		else if(m_oBoolDict[EKey.IS_INIT] && AppleAuthManager.IsCurrentPlatformSupported) {
	...
	public void LogoutWithApple(System.Action<CServicesManager> a_oCallback) {
		...
		try {
			...
			// 로그인 되었을 경우
			if(m_oBoolDict[EKey.IS_INIT] && m_oBoolDict[EKey.IS_APPLE_LOGIN]) {
				m_oBoolDict[EKey.IS_APPLE_LOGIN] = false;
			}
#endif // #if UNITY_IOS && APPLE_LOGIN_ENABLE
		} finally {
	...
	private void OnLoginWithApple(ICredential a_oCredential) {
		...
		m_oBoolDict[EKey.IS_APPLE_LOGIN] = oAppleIDCredential != null;

		CFunc.ShowLog($"CServicesManager.OnLoginWithApple: {m_oBoolDict[EKey.IS_APPLE_LOGIN]}", KCDefine.B_LOG_COLOR_PLUGIN);

		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_SERVICES_M_LOGIN_WITH_APPLE_CALLBACK, () => {
			CCommonAppInfoStorage.Inst.AppInfo.AppleUserID = m_oBoolDict[EKey.IS_APPLE_LOGIN] ? oAppleIDCredential.User : string.Empty;
			CCommonAppInfoStorage.Inst.AppInfo.AppleIDToken = m_oBoolDict[EKey.IS_APPLE_LOGIN] ? System.Text.Encoding.Default.GetString(oAppleIDCredential.IdentityToken, KCDefine.B_VAL_0_INT, oAppleIDCredential.IdentityToken.Length) : string.Empty;

			CCommonAppInfoStorage.Inst.SaveAppInfo();
			m_oCallbackDict01.GetValueOrDefault(EServicesCallback.LOGIN_WITH_APPLE)?.Invoke(this, m_oBoolDict[EKey.IS_APPLE_LOGIN]);
		});
	}
	...
	private void OnUpdateAppleLoginState(CredentialState a_eState) {
		m_oBoolDict[EKey.IS_APPLE_LOGIN] = a_eState == CredentialState.Authorized;
		CFunc.ShowLog($"CServicesManager.OnUpdateAppleLoginState: {m_oBoolDict[EKey.IS_APPLE_LOGIN]}", KCDefine.B_LOG_COLOR_PLUGIN);

		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_SERVICES_M_UPDATE_APPLE_LOGIN_STATE_CALLBACK, () => {
			CCommonAppInfoStorage.Inst.AppInfo.AppleUserID = m_oBoolDict[EKey.IS_APPLE_LOGIN] ? this.AppleUserID : string.Empty;
			CCommonAppInfoStorage.Inst.AppInfo.AppleIDToken = m_oBoolDict[EKey.IS_APPLE_LOGIN] ? this.AppleIDToken : string.Empty;

			CCommonAppInfoStorage.Inst.SaveAppInfo();
			m_oCallbackDict01.GetValueOrDefault(EServicesCallback.UPDATE_APPLE_LOGIN_STATE)?.Invoke(this, m_oBoolDict[EKey.IS_APPLE_LOGIN]);
		});
	}
