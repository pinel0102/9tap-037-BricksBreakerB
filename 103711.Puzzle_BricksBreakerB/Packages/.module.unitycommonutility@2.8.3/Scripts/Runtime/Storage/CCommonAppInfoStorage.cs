...
public partial class CCommonAppInfo : CCommonBaseInfo {
	...
	#region 프로퍼티
	[IgnoreMember] public bool IsAgree { get { return bool.Parse(m_oStrDict.GetValueOrDefault(KEY_IS_AGREE, KCDefine.B_TEXT_FALSE)); } set { m_oStrDict.ExReplaceVal(KEY_IS_AGREE, $"{value}"); } }
	[IgnoreMember] public bool IsAgreeTracking { get { return bool.Parse(m_oStrDict.GetValueOrDefault(KEY_IS_AGREE_TRACKING, KCDefine.B_TEXT_FALSE)); } set { m_oStrDict.ExReplaceVal(KEY_IS_AGREE_TRACKING, $"{value}"); } }
	[IgnoreMember] public bool IsFirstPlay { get { return bool.Parse(m_oStrDict.GetValueOrDefault(KEY_IS_FIRST_PLAY, KCDefine.B_TEXT_FALSE)); } set { m_oStrDict.ExReplaceVal(KEY_IS_FIRST_PLAY, $"{value}"); } }
	[IgnoreMember] public bool IsEnableShowTrackingDescPopup { get { return bool.Parse(m_oStrDict.GetValueOrDefault(KEY_IS_ENABLE_SHOW_TRACKING_DP, KCDefine.B_TEXT_FALSE)); } set { m_oStrDict.ExReplaceVal(KEY_IS_ENABLE_SHOW_TRACKING_DP, $"{value}"); } }

	[IgnoreMember] public int AppRunningTimes { get { return int.Parse(m_oStrDict.GetValueOrDefault(KEY_APP_RUNNING_TIMES, KCDefine.B_STR_0_INT)); } set { m_oStrDict.ExReplaceVal(KEY_APP_RUNNING_TIMES, $"{value}"); } }
	[IgnoreMember] public string DeviceID { get { return m_oStrDict.GetValueOrDefault(KEY_DEVICE_ID, string.Empty); } set { m_oStrDict.ExReplaceVal(KEY_DEVICE_ID, value); } }
	[IgnoreMember] public System.DateTime InstallTime { get { return this.InstallTimeStr.ExIsValid() ? this.CorrectInstallTimeStr.ExToTime(KCDefine.B_DATE_T_FMT_SLASH_YYYY_MM_DD_HH_MM_SS) : System.DateTime.Now; } set { m_oStrDict.ExReplaceVal(KEY_INSTALL_TIME, value.ExToLongStr()); } }
	[IgnoreMember] public System.DateTime PrevPlayTime { get { return this.PrevPlayTimeStr.ExIsValid() ? this.CorrectPrevPlayTimeStr.ExToTime(KCDefine.B_DATE_T_FMT_SLASH_YYYY_MM_DD_HH_MM_SS) : System.DateTime.Now; } set { m_oStrDict.ExReplaceVal(KEY_PREV_PLAY_TIME, value.ExToLongStr()); } }
...
public partial class CCommonAppInfoStorage : CSingleton<CCommonAppInfoStorage> {
	#region 프로퍼티
	public bool IsFirstStart { get; private set; } = false;
...
#if !UNITY_EDITOR && (UNITY_IOS || UNITY_ANDROID) && NEWTON_SOFT_JSON_MODULE_ENABLE
...
#endif // #if !UNITY_EDITOR && (UNITY_IOS || UNITY_ANDROID) && NEWTON_SOFT_JSON_MODULE_ENABLE
	}
#endif // #if STORE_VER_CHECK_ENABLE
	#endregion // 조건부 함수
}
