...
public partial class CCommonUserInfo : CCommonBaseInfo {
	...
	#region 프로퍼티
	[IgnoreMember] public EUserType UserType { get { return (EUserType)int.Parse(m_oStrDict.GetValueOrDefault(KEY_USER_TYPE, $"{(int)EUserType.NONE}")); } set { m_oStrDict.ExReplaceVal(KEY_USER_TYPE, $"{(int)value}"); } }
	#endregion // 프로퍼티
