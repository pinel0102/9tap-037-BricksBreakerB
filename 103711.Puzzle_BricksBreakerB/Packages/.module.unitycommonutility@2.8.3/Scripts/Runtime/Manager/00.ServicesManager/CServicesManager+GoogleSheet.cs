...
public partial class CServicesManager : CSingleton<CServicesManager> {
	...
	private (int, int, string, string) MakeLoadCellInfo(int a_nSrcIdx, int a_nNumRows) {
		...
		return (nSrcIdx, nNumCells, string.Format(KCDefine.U_CELL_N_FMT_GOOGLE_SHEET_SRC, nSrcIdx), string.Format(KCDefine.U_CELL_N_FMT_GOOGLE_SHEET_DEST, a_nSrcIdx + nNumCells));
	}
