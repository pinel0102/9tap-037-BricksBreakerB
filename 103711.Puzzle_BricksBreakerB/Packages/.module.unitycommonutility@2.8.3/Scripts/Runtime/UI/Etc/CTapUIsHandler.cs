...
public partial class CTapUIsHandler : CComponent {
	...
	public int SelTapBtnIdx => m_oIntDict.GetValueOrDefault(EKey.SEL_TAP_BTN_IDX);
	...
	private void UpdateUIsState() {
		for(int i = 0; i < m_oTapBtnList.Count; ++i) {
			m_oContentsUIsList[i].SetActive(i == m_oIntDict.GetValueOrDefault(EKey.SEL_TAP_BTN_IDX));
		}
	}

	/** 탭 버튼을 눌렀을 경우 */
	protected virtual void OnTouchTapBtn(int a_nIdx) {
		m_oIntDict.ExReplaceVal(EKey.SEL_TAP_BTN_IDX, a_nIdx);
