...
namespace LevelEditorScene {
				...
				for(int i = 0; i < a_stCellObjInfo.m_stSize.y; ++i) {
					for(int j = 0; j < a_stCellObjInfo.m_stSize.x; ++j) {
						...
						if(stIdx.Equals(a_stIdx)) {
							...
							stCellInfo.m_oCellObjInfoList.ExAddVal(Factory.MakeEditorCellObjInfo(EObjKinds.BG_PLACEHOLDER_01, Vector3Int.one, a_stIdx, currentColorID));
						}
					}
				}
			}
			...
			for(int i = 0; i < this.SelLevelInfo.m_oCellInfoDictContainer.Count; ++i) {
				...
				if(this.SelLevelInfo.m_oCellInfoDictContainer[i].TryGetValue(a_nIdx, out STCellInfo stCellInfo)) {
					...
					if(Input.GetMouseButtonUp((int)EMouseBtn.LEFT) && m_oObjKindsDict[EKey.SEL_OBJ_KINDS].ExIsValid()) {
						this.AddCellObjInfo(Factory.MakeEditorCellObjInfo(m_oObjKindsDict[EKey.SEL_OBJ_KINDS], this.GetEditorObjSize(), stIdx, currentColorID), stIdx, Input.GetKey(KeyCode.LeftShift));
					}
					// 객체 제거가 가능 할 경우
	...
	public partial class CSubLevelEditorSceneManager : CLevelEditorSceneManager, IEnhancedScrollerDelegate {
        ...
        private void OnTouchREUIsPageUIs01ScoreBtn() {
            ...
            this.SelLevelInfo.Score1 = bIsValid01 ? score1 : this.SelLevelInfo.Score1;
            this.SelLevelInfo.Score2 = bIsValid02 ? score2 : this.SelLevelInfo.Score2;
