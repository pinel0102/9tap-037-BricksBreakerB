...
/// <para>상위 버전이 올라가면 하위 버전은 0부터 시작합니다.</para>
...
public class VersionManager
{
    ...
    {		
        public ProjecInfo(bool useProjectManager = false)
        {
            ...
            else
            {
                ...
                storeURLAndroid = string.Format(ProjectManager.storeURL_AOS, ProjectManager.appIdentifier);
                storeURLiOS = string.Format(ProjectManager.storeURL_iOS, ProjectManager.appStoreConnectId);
    ...
    private static void LogVersionManager()
    {
        if (useManager)
        {
            Debug.Log(CodeManager.GetMethodName() + string.Format("<color=#FFFF00>Version Manager : ver {0} ({1})</color>", PlayerSettings.bundleVersion, PlayerSettings.Android.bundleVersionCode));
    ...
    private static void OverrideProjInfoTable(string newVersion, int newBuildNumber)
    {
        if (overrideProjInfoTable)
        {
            if (CPlatformOptsSetter.ProjInfoTable != null)
            {
                ...
                STCompanyInfo _companyInfo = new STCompanyInfo() {
                    m_oCompany = _pInfo.companyName,
                    m_oPrivacyURL = _pInfo.privacyURL,
                    m_oServicesURL = _pInfo.servicesURL,
                    m_oSupportsMail = _pInfo.supportsMail
                };

                STCommonProjInfo _commonProjInfo = new STCommonProjInfo() {
                    m_oAppID = _pInfo.appIdentifier,
                    m_oProjName = _pInfo.projectName,
                    m_oProductName = _pInfo.productName,
                    m_oExtraProjDirName = _pInfo.projectName
                };

                STBuildVerInfo _buildVerInfo = new STBuildVerInfo() {
                    m_nNum = newBuildNumber,
                    m_oVer = newVersion,
                };

                STProjInfo _projInfo_iOS = new STProjInfo() {
                    m_stBuildVerInfo = _buildVerInfo,
                    m_oAppID = _pInfo.appIdentifier,
                    m_oStoreAppID = _pInfo.appStoreConnectId
                };
                ...
                oProjInfoTable.SetCommonProjInfo(_commonProjInfo);
                oProjInfoTable.SetiOSAppleProjInfo(_projInfo_iOS);
                oProjInfoTable.SetAndroidGoogleProjInfo(_projInfo_Common);
                oProjInfoTable.SetAndroidAmazonProjInfo(_projInfo_Common);
                oProjInfoTable.SetStandaloneMacSteamProjInfo(_projInfo_Common);
                oProjInfoTable.SetStandaloneWndsSteamProjInfo(_projInfo_Common);
