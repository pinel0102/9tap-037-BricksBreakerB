                ...
                if (boosterList[i] == true)
                {
                    ...
                    if (targetList.Count > 0)
                    {
                        if (boosterItem[i])
                        {
                            switch(i)
                            {
                                case 0: Debug.Log(CodeManager.GetMethodName() + string.Format("[USE BOOSTER] {0} ({1} -> {2})", EItemKinds.BOOSTER_ITEM_01_MISSILE, GlobalDefine.UserInfo.Booster_Missile, GlobalDefine.UserInfo.Booster_Missile - 1));
                                        GlobalDefine.UserInfo.Booster_Missile = Mathf.Max(0, GlobalDefine.UserInfo.Booster_Missile - 1); break;
                                case 1: Debug.Log(CodeManager.GetMethodName() + string.Format("[USE BOOSTER] {0} ({1} -> {2})", EItemKinds.BOOSTER_ITEM_02_LIGHTNING, GlobalDefine.UserInfo.Booster_Lightning, GlobalDefine.UserInfo.Booster_Lightning - 1)); 
                                        GlobalDefine.UserInfo.Booster_Lightning = Mathf.Max(0, GlobalDefine.UserInfo.Booster_Lightning - 1); break;
                                case 2: Debug.Log(CodeManager.GetMethodName() + string.Format("[USE BOOSTER] {0} ({1} -> {2})", EItemKinds.BOOSTER_ITEM_03_BOMB, GlobalDefine.UserInfo.Booster_Bomb, GlobalDefine.UserInfo.Booster_Bomb - 1));
                                        GlobalDefine.UserInfo.Booster_Bomb = Mathf.Max(0, GlobalDefine.UserInfo.Booster_Bomb - 1); break;
                            }
                        }
                        else if (boosterRuby[i])
                        {
                            switch(i)
                            {
                                case 0: Debug.Log(CodeManager.GetMethodName() + string.Format("[BUY BOOSTER] {0} ({1} -> {2})", EItemKinds.BOOSTER_ITEM_01_MISSILE, GlobalDefine.UserInfo.Ruby, GlobalDefine.UserInfo.Ruby - GlobalDefine.CostRuby_Booster)); break;
                                case 1: Debug.Log(CodeManager.GetMethodName() + string.Format("[BUY BOOSTER] {0} ({1} -> {2})", EItemKinds.BOOSTER_ITEM_02_LIGHTNING, GlobalDefine.UserInfo.Ruby, GlobalDefine.UserInfo.Ruby - GlobalDefine.CostRuby_Booster)); break;
                                case 2: Debug.Log(CodeManager.GetMethodName() + string.Format("[BUY BOOSTER] {0} ({1} -> {2})", EItemKinds.BOOSTER_ITEM_03_BOMB, GlobalDefine.UserInfo.Ruby, GlobalDefine.UserInfo.Ruby - GlobalDefine.CostRuby_Booster)); break;
                            }
